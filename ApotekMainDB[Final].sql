...
DROP TRIGGER NOURUT_BI;
DROP FUNCTION GETNOJUAL;
DROP FUNCTION GETNORESEP;
DROP FUNCTION GETURUTAN;
...
CREATE TABLE DJUALOBAT(
    NOJUAL VARCHAR2(5) CONSTRAINT FK_DJUAL  REFERENCES HJUALOBAT(NOJUAL),
...
CREATE OR REPLACE TRIGGER NOURUT_BI
BEFORE INSERT ON DETAILRESEP FOR EACH ROW
DECLARE
	NUM NUMBER;
	TEMP VARCHAR2(5);
BEGIN
...
CREATE OR REPLACE FUNCTION GETNOJUAL(T VARCHAR2)
RETURN VARCHAR2
IS 
	NUM NUMBER;
	X VARCHAR2(5);
...
CREATE OR REPLACE FUNCTION GETNORESEP(T VARCHAR2)
RETURN VARCHAR2
IS 
	NUM NUMBER;
	X VARCHAR2(5);
...
CREATE OR REPLACE FUNCTION GETURUTAN(T VARCHAR2)
RETURN NUMBER
IS 
	NUM NUMBER;
BEGIN
...
CREATE OR REPLACE VIEW HOBAT AS
SELECT NOJUAL NOTA,NAMAPEGAWAI PEGAWAI,TGLJUAL TANGGAL ,TOTALJUAL TOTAL FROM HJUALOBAT , PEGAWAI WHERE HJUALOBAT.KDPEGAWAI = PEGAWAI.KDPEGAWAI;

CREATE OR REPLACE VIEW HRESEP AS
SELECT NORESEP NOTA ,NAMACUST CUSTOMER, NAMADOKTER DOKTER,NAMAPEGAWAI PEGAWAI ,TGLRESEP TANGGAL,TOTALRESEP TOTAL , DECODE(STATUSKIRIM,1,'TERKIRIM',0,'BELUM TERKIRIM') AS STATUS FROM HJUALRESEP , PEGAWAI ,DOKTER,CUSTOMER 
WHERE HJUALRESEP.KDPEGAWAI = PEGAWAI.KDPEGAWAI 
AND HJUALRESEP.KDCUST = CUSTOMER.KDCUST
AND HJUALRESEP.KDDOKTER = DOKTER.KDDOKTER;

CREATE OR REPLACE VIEW DOBAT AS
SELECT NOJUAL NOTA,NAMAOBAT OBAT,JUMLAHOBAT QTY, HARGAJUALOBAT HARGA FROM DJUALOBAT,OBAT
WHERE DJUALOBAT.KDOBAT = OBAT.KDOBAT;  

CREATE OR REPLACE VIEW DRESEP AS
SELECT NORESEP NOTA,NAMAOBAT OBAT,URUTAN NOMOR_URUT, JUMLAHPAK QTY ,HARGASATUAN HARGA_SATUAN,CARAMINUM CARA_MINUM FROM DETAILRESEP,OBAT
WHERE DETAILRESEP.KDOBAT = OBAT.KDOBAT; 
...
insert into detailresep values('NR001','HD001',1,12,122545,'3x1');
insert into detailresep values('NR002','PU001',2,8,63355,'2x1');
insert into detailresep values('NR003','GL001',3,2,105983,'3x1');
insert into detailresep values('NR004','TO001',4,3,52234,'3x1');
insert into detailresep values('NR005','NA001',5,2,127461,'2x1');
